...
class Tables_Test(unittest.TestCase):
    ...
    def test_02_nat_prerouting(self):
        ...
        """
        tables = Tables("")
        line = "ip6tables -t nat -A PREROUTING -s 2001:db8:dead:1::/64"
        line = line + " -p tcp --dport   80 -j SNAT --to-source 2001:db8:feed::2"
        tables.put_into_tables(line)
        expect = ['-A PREROUTING -s 2001:db8:dead:1::/64 -p tcp --dport 80 -j SNAT --to-source 2001:db8:feed::2 ']
